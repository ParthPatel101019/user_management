...
async def test_get_by_title_event_exists(db_session, company_tour_event):
    retrieved_event = await EventService.get_by_title(
        db_session, company_tour_event.title
    )
    assert retrieved_event is not None
...
async def test_update_event_valid_data(db_session, company_tour_event):
    ...
    updated_event = await EventService.update(
        db_session, company_tour_event.id, update_data
    )
...
async def test_list_events_with_pagination(
    db_session, company_tour_event, mock_interview_event, guest_lecture_event
):
...
async def test_publish_event(db_session, company_tour_event):
    ...
    published_event = await EventService.publish_event(
        db_session, company_tour_event.id
    )
...
async def test_unpublish_event(db_session, mock_interview_event):
    ...
    unpublished_event = await EventService.unpublish_event(
        db_session, mock_interview_event.id
    )
...
async def test_list_events_by_creator(
    db_session, verified_user, manager_user, company_tour_event, mock_interview_event
):
    events_by_verified_user = await EventService.list_events_by_creator(
        db_session, verified_user.id
    )
    events_by_manager_user = await EventService.list_events_by_creator(
        db_session, manager_user.id
    )
...
async def test_list_published_events(
    db_session, company_tour_event, mock_interview_event
):
    mock_interview_event.published = True
    await db_session.commit()
...
async def test_list_events_by_type(
    ...
    company_tour_events = await EventService.list_events_by_type(
        db_session, EventType.COMPANY_TOUR
    )
    mock_interview_events = await EventService.list_events_by_type(
        db_session, EventType.MOCK_INTERVIEW
    )
    guest_lecture_events = await EventService.list_events_by_type(
        db_session, EventType.GUEST_LECTURE
    )
