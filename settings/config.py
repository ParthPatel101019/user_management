...
class Settings(BaseSettings):
    max_login_attempts: int = Field(
        default=10, description="Background color of QR codes"
    )
    # Server configuration
    server_base_url: AnyUrl = Field(
        default="http://localhost:8000", description="Base URL of the server"
    )
    ...
    access_token_expire_minutes: int = Field(
        default=30000, description="Expiration time for access tokens in minutes"
    )
    ...
    postgres_server: str = Field(
        default="postgres", description="PostgreSQL server address"
    )
    ...
    smtp_server: str = Field(
        default="smtp.mailtrap.io", description="SMTP server for sending emails"
    )
    smtp_port: int = Field(default=2525, description="SMTP port for sending emails")
    smtp_username: str = Field(
        default="your-mailtrap-username", description="Username for SMTP server"
    )
    smtp_password: str = Field(
        default="your-mailtrap-password", description="Password for SMTP server"
    )
    account_verfiy_destination: str = Field(
        default="http://localhost:8000/",
        description="Where to redirect the user after account verification",
    )
